import pytest
from pyxivapi import XIVAPIClient, CustomError
...
def test_search_exact_name(client: XIVAPIClient):
...
def test_search_partial_name(client: XIVAPIClient):
...
def test_search_numeric(client: XIVAPIClient):
...
def test_search_invalid_syntax(client: XIVAPIClient):
    with pytest.raises(CustomError):
